...
export class ReservationController {
  ...
  const query = `
    INSERT INTO reserva (habitacion_id, numero_noches, cliente_id)
    VALUES ($1, $2, $3);
  `;
  ...
  const query = "UPDATE reserva SET estado_id = $1 WHERE id = $2";
  ...
  // Consulta SQL para agrupar reservas por estado y orden personalizado
  const query = `
    SELECT 
    ...
    ORDER BY 
      CASE
        WHEN er.estado ILIKE 'completada' THEN 1
        WHEN er.estado ILIKE 'pendiente' THEN 2
        WHEN er.estado ILIKE 'activa' THEN 3
        WHEN er.estado ILIKE 'cancelada' THEN 4
        ELSE 5
      END DESC,
      er.estado ASC;
  `;
